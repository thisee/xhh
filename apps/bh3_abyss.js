...
export class bh3_abyss extends plugin {
  ...
  async _abyss(e, apiList) {
    ...
    const reportList = reports.slice(0, 8).map((r, ri) => {
      ...
      return `<div class="report-card"><div class="card-header"><div class="level-badge">${fmtLevel(r.level)}</div><div class="score-wrap"><div class="score">${r.score || 0}</div><div class="boss-name-right">${bossName}</div></div></div><div class="card-body"><div class="lineup-wrap"><div class="lineup">${lined}${elfHtml}</div></div><div class="boss-area"><div class="boss-icon">${bossIcon}</div><div class="right-info"><div class="ri-line">#${r.rank || 0}</div><div class="ri-line">${cupText}${cupChange}</div><div class="ri-line time">${fmtTs(r.updated_time_second)}</div></div></div></div></div>`;
    }).join('\n');
