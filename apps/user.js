import { mhy, yaml, QR, sleep, reply_recallMsg, recallMsg, makeForwardMsg, api, config } from '#xhh'
...
export class user extends plugin {
  ...
  async sm(e) {
    ...
    if (last_time && !e.isMaster) {
      ...
      e.reply('扫码登录CD中，请不要重复触发！剩余时间：' + (CD - seconds) + '秒', true)
      return true
    }
    ...
    let res = await fetch(url, { method: "POST", headers, body: JSON.stringify(body) }).then(res => res.json())

    const sm_url = res.data.url

    let img = segment.image((await QR.toDataURL(sm_url)).replace("data:image/png;base64,", "base64://"))

    const re = await reply_recallMsg(e, ['请在60秒内使用手机米游社扫码登录', img, '调用[未定事件铺]接口,获取米游社game_token,谁触发谁扫码,请不要帮别人绑定自己的米游社！！！'], 60, true)
    ...
    for (var n = 1; n < 150; n++) {
      await sleep(1000)
      res = await fetch(url, { method: "POST", headers, body: JSON.stringify(body) }).then(res => res.json())
      ...
      if (res.data.stat == "Confirmed") {
        ...
        headers.Cookie = ck
        const { sendMsg, ltoken } = await mhy.refresh_cookies(e, headers, SToken, stuid)
  ...
  async refresh_ck(e) {
    ...
    for (let k in data_) {
      ...
      const { sendMsg, ltoken } = await mhy.refresh_cookies(e, headers, SToken, k)
      if (ltoken && data) {
        for (let m in data) {
  ...
  async process_files(yaml_url, data_) {
    if (!fs.existsSync(yaml_url)) {
      ...
      for (let k in data_) {
        if (_data_[k]?.hasOwnProperty('is_sign')) data_[k]['is_sign'] = _data_[k].is_sign
        _data_[k] = data_[k]
  ...
  async mysReqErrHandler_(e, args, reject) {
    ...
    if (![1034, 5003, 10035, 10041, 10104].includes(Number(args?.res?.retcode))) {
      return reject()
    }
