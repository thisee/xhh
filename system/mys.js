  ...
  async data(name = '', type = 'js', isSr = false) {
    ...
    if (name) {
      ...
      for (let va of list) {
        ...
        //    }
        //   }
        if (va.title.replace(/ /g, '') == name) return { id };
      }
      return false;
    } else {
      let names = [],
      ...
      for (let n in list) {
        ...
        if (!['syw', 'yq'].includes(type)) {
          ...
          if (type != 'wq') {
            for (let s of text) {
              if (s.includes('星级')) jis.push(s.replace(/星级\//, ''));
              if (s.includes('元素')) yuanshus.push(s.replace(/元素\//, ''));
            ...
            for (let s of text) {
              ...
              if (s.includes('武器类型'))
                wuqis.push(s.replace(/武器类型\//, ''));
            }
          }
        }
      }
      // 图鉴别名补缺
      //  const pa='./plugins/xhh/system/default/gs_js_names.yaml'
