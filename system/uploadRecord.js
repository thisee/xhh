...
async function uploadRecord(
  ...
  if (!Bot.sendUni) throw new Error('当前协议不支持uploadRecord上传');
  const result = await getPttBuffer(
    record_url,
    Bot?.config?.ffmpeg_path || 'ffmpeg',
    transcoding
  );
  if (!result.buffer) {
    return false;
  ...
  const body = core.pb.encode({
    ...
    5: {
      ...
      9: 4,
      11: 0,
      10: Bot?.apk?.version || Bot?.apk?.version,
      12: 1,
      13: 1,
      14: codec,
      15: 1,
  ...
  const headers = {
    'User-Agent': `QQ/${Bot?.apk?.version || Bot?.apk?.version} CFNetwork/1126`,
    'Net-Type': 'Wifi',
  };
  await fetch(url, {
    method: 'POST', //post请求
...
(function (ErrorCode) {
  ...
  /** 不支持的file类型(没有流) */
  ErrorCode[(ErrorCode['HighwayFileTypeError'] = -140)] =
    'HighwayFileTypeError';
  /** 文件安全校验未通过不存在 */
  ErrorCode[(ErrorCode['UnsafeFile'] = -150)] = 'UnsafeFile';
  /** 离线(私聊)文件不存在 */
  ErrorCode[(ErrorCode['OfflineFileNotExists'] = -160)] =
    'OfflineFileNotExists';
  /** 群文件不存在(无法转发) */
