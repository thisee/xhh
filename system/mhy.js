...
class mhy {
 ...
 async shebei(e,info){
   if(info?.device_fp && info?.device_id) return this.bd(e,yaml_url,info.device_id,info.device_fp)
    ...
    if(oaid.includes('error')) return e.reply('设备oaid获取失败，可能你的设备不支持获取')
  let body={
    device_id: oaid,
...
let res = await fetch(this.fp_url,{method: "POST", headers, body: JSON.stringify(body)}).then(res => res.json())
  let fp
  ...
  const res1 = await fetch('https://bbs-api.miyoushe.com/apihub/api/deviceLogin',{method: "POST", headers, body: JSON.stringify(body)}).then(res => res.json())
  const res2 = await fetch('https://bbs-api.miyoushe.com/apihub/api/saveDevice',{method: "POST", headers, body: JSON.stringify(body)}).then(res => res.json())
  if(res1.retcode==0&&res2.retcode==0) {
    return this.bd(e,device_id,fp,info)
  }else{ logger.error(res1,res2) }
   return
}
...
//绑定设备参数保存到文件
   ...
   getUser(e){
   const user=e.user.getMysUser() || e.user.getMysUser('sr') || e.user.getMysUser('zzz')
   return user
   }
 ...
 'x-rpc-device_name': info ? info.deviceFingerprint.split('/')[0]+' '+info.deviceModel : this.randomString(lodash.random(1, 10)),
...
async refresh_cookies(e,headers,SToken,id){
  ...
  let urls=[`https://api-takumi.mihoyo.com/auth/api/getCookieAccountInfoBySToken?stoken=${SToken}&uid=${id}`,'https://passport-api.mihoyo.com/account/auth/api/getLTokenBySToken']
 ...
 let userck = (await import(`file://${process.cwd()}/plugins/genshin/model/user.js`)).default
 e.ck = e.msg;
...
async getSToken(e){
    ...
    if (fs.existsSync(path2)) {
      ...
      data=yaml.get(path2)
      for (let k in data) {
      data_[data[k].stuid] = [data[k].stoken,`stuid=${data[k].stuid};stoken=${data[k].stoken};mid=${data[k].mid};`,data[k].mid,data[k].ltoken]
      }
      /*
          hk4e_cn国服原神
          hkrpg_cn国服星铁
